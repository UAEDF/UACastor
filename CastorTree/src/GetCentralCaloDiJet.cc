//-- Description: Function to retrieve central Calo Di Jet information (original author: Alexander FlossDorf)
...
void CastorTree::GetCentralCaloDiJet(const vector<MyJet>& JetVector, MyDiJet& dijet) {
  ...
  if(posJet1 >= 0 && posJet2 >= 0) {
    ...
    //-- jet 1 selection
    if(jet1.pt_cal > jetPtCut_ && fabs(jet1.eta_cal) < jetEtaCut_ && jet1.LooseCaloJetId == true) accept_jet1 = true;
    
    //-- jet 2 selection
    if(jet2.pt_cal > jetPtCut_ && fabs(jet2.eta_cal) < jetEtaCut_ && jet2.LooseCaloJetId == true) accept_jet2 = true;
  ...
  if(CentralCaloDiJetDebug) dijet.Print();
