// -*- C++ -*-
//
// Package:    TreeMaker7TeV
// Class:      TreeMaker7TeV
// 
/**\class TreeMaker7TeV TreeMaker7TeV.cc Analyzers/TreeMaker7TeV/src/TreeMaker7TeV.cc
...
// $Id: TreeMaker7TeV.cc,v 1.3 2010/07/05 09:49:46 roland Exp $
//
//
...
void TreeMaker7TeV::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  if(debug) cout<<"Run "<< iEvent.id().run() <<" Event "<<iEvent.id().event()<<endl;
   ...
   if(digiCollection.isValid()){
     ...
     for (unsigned int digiItr=0; digiItr<digiCollection->size(); ++digiItr) { //loop over 224 channels
       ...
       for (Int_t sample = 0 ; sample < nrSamples; ++sample) { //loop over 10 time slices
	 ...
	 cout<<"array entry: "<<i+1<<", module: "<<module[i]<<", sector: "<<sector[i]<<" ,channel: "<<channel[i] 
	     <<", signal in 4 last time slices: "<<sumADC[i]<<endl;
       }
       getchar();
     }
   ...
   if(hlTriggers.failedToGet()) {
     ...
     if(triggerName.triggerName(trigpos) == "HLT_PhysicsDeclared") {
       ...
       if(trigAccept == true) PhysicsDeclared = 1;
       else PhysicsDeclared = 0;
     }
   ...
   if (L1GTRR.isValid()) {
     ...
     for (unsigned int iBit = 0; iBit < numberTechnicalTriggerBits; ++iBit) {
       ...
       if(debug) cout<<"L1TechnicalTriggers "<<iBit+1<<" has value "<<techTrigger<<endl;
     }
   } 
   else {
...
DEFINE_FWK_MODULE(TreeMaker7TeV);
