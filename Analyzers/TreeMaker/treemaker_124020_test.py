...
process.TFileService = cms.Service("TFileService",
   fileName = cms.string("CentralJetFilters_Jan23RawReco_Run124020_test.root")
)
...
process.treemaker = cms.EDAnalyzer('TreeMaker',
      OutputFileName = cms.string("TreeMaker_Jan23RawReco_Run124020_test.root"),
      CaloJetCorrector = cms.string('L2L3JetCorrectorAK5Calo'),
      PFJetCorrector = cms.string('L2L3JetCorrectorAK5PF'),
      HLTriggerResults = cms.InputTag( 'TriggerResults','','HLT' ),
      CaloJets = cms.InputTag('ak5CaloJets', '', 'RERECO'),
...
#process.p = cms.Path(process.physDecl*process.hltLevel1GTSeed*process.primaryVertexFilter*process.noscraping*process.castorDigis*(process.calomonojetSelector+process.calodijetSelector+process.pfmonojetSelector+process.pfdijetSelector)*process.treemaker)

process.p = cms.Path(process.bit40*process.bptxAnd*process.physDecl*process.oneGoodVertexFilter*process.noScraping*process.castorDigis*(process.calomonojetSelector+process.calodijetSelector+process.pfmonojetSelector+process.pfdijetSelector)*process.treemaker)
